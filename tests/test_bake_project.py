...
def test_year_compute_in_license_file(cookies):
    with bake_in_temp_dir(cookies, extra_context={"open_source_license": "MIT license"}) as result:
...
def project_info(result):
    """Get toplevel dir, project_slug, and package_name from baked cookies"""
    project_path = str(result.project)
    project_slug = os.path.split(project_path)[-1]
    package_name = project_slug.replace('-', '')
    package_dir = os.path.join(project_path, package_name)
    return project_path, project_slug, package_name, package_dir
...
def test_using_pytest(cookies):
    with bake_in_temp_dir(
        ...
        test_file_path = result.project.join(
            'tests/test_pythonboilerplate.py'
        )
